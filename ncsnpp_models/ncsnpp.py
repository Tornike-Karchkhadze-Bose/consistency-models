...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config, cfg):
    ...
    self.cfg = cfg
    self.label_dim       = 0
    self.sigma_min       = 0.002               # Minimum supported noise level.
    self.sigma_max       = 80   # Maximum supported noise level.
    self.sigma_data      = 0.5              # Expected standard deviation of the training data.
    self.rho             = 7
    self.use_fp16        = False
    ...
    # self.dummy_param = nn.Parameter(torch.tensor(0.0)) ### this it for the case if you need to debug and accelerator doen't work 
  ...
  def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
      ...
      dtype = torch.float32 # if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda') else torch.float32
      ...
      F_x = self.forward_((c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
      assert F_x.dtype == dtype
      D_x = c_skip * x + c_out * F_x.to(torch.float32)
      return D_x


  def forward_(self, x, time_cond, class_labels=None, augment_labels=None):

    # return x + self.dummy_param * x ### this it for the case if you need to debug and accelerator doen't work 
    # timestep/noise_level embedding; only for continuous training
